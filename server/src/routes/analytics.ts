...
export default async function analyticsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/dashboard/:userId', async (request: FastifyRequest<{
    ...
    const stats = {
      totalVitals: 28,
      activeAlerts: 2,
      weeklyTrend: 15, // percentage increase/decrease
  ...
  fastify.get('/pregnancy/progress/:userId', async (request: FastifyRequest<{
    Params: { userId: string }
  }>, reply: FastifyReply) => {
    const { userId } = request.params;
    
    // Mock pregnancy progress data
    const progress = {
      weeksPregnant: 24,
      daysRemaining: 112,
  ...
  fastify.get('/trends/:userId', async (request: FastifyRequest<{
    Params: { userId: string };
    Querystring: { period?: 'week' | 'month' | '3months' }
  }>, reply: FastifyReply) => {
    const { userId } = request.params;
  ...
  fastify.get('/insights/:userId', async (request: FastifyRequest<{
    Params: { userId: string }
  }>, reply: FastifyReply) => {
    const { userId } = request.params;
